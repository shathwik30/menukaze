...
export async function createStationAction(raw: unknown): Promise<ActionResult<{ id: string }>> {
  ...
  try {
    return await withRestaurantAction(['kds.configure'], async ({ restaurantId }) => {
    ...
    return actionError(error, 'Failed to create station.', STATION_PERMISSION_ERROR);
  }
}
...
export async function updateStationAction(raw: unknown): Promise<ActionResult> {
  ...
  try {
    ...
    return actionError(error, 'Failed to update station.', STATION_PERMISSION_ERROR);
  }
}
...
export async function archiveStationAction(stationId: string): Promise<ActionResult> {
  ...
  try {
    ...
    return actionError(error, 'Failed to archive station.', STATION_PERMISSION_ERROR);
  }
}
...
export async function setItemStationsAction(raw: unknown): Promise<ActionResult> {
  ...
  try {
    ...
    return actionError(error, 'Failed to update item routing.', STATION_PERMISSION_ERROR);
  }
}
...
/**
 * Advance the per-line status for one or more lines on a single order. Used by
 * a station-filtered KDS to mark "this station's lines are done". When every
 * line on the order is `ready`, the order itself moves to `ready`.
 */
export async function advanceOrderLinesAction(raw: unknown): Promise<ActionResult> {
  ...
  try {
    return await withRestaurantAction(['kds.update'], async ({ restaurantId }) => {
      const conn = await getMongoConnection('live');
    ...
    return actionError(error, 'Failed to update order line.', KDS_PERMISSION_ERROR);
  }
}
