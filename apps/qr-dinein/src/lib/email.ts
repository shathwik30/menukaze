 ...
 * qr-dinein app. Honours `MENUKAZE_SKIP_EMAIL=true` so local smoke tests
 * run offline without consuming Resend quota, and logs the
 * subject/recipient of each send for debugging.
 */
...
export interface SendTransactionalInput {
  ...
  replyTo?: string;
  /** React element to render — templates in apps/qr-dinein/src/emails. */
  react: ReactElement;
}
